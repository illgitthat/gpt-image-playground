...
### App controls and the tested gateway
...
The playground exposes only `auto`, `low`, `medium`, and `high` quality and the sizes `auto`, `1024x1024`, `1536x1024`, and `1024x1536`. The maintainer's gateway rejected `xhigh`. A September 11, 2026 recheck rejected `2048x2048` and `3840x2160` for both 2.5 deployment headers. These observed gateway restrictions do not override the broader model capabilities above or establish limits for other deployments.
...
The guide describes `n` for variations in Images API examples; it does not establish a Responses-tool batch limit. The 2 RPM quota is specific to the maintainer's personal deployment. The app currently enforces 2 requests/minute/model and 2 images/batch for every backend; these are app settings, not universal provider limits. Respect upstream retry guidance even when a local request is allowed. The guide's runnable example is still pinned to an older model; this app requests only Flare or Sunburst.
