...
import { Copy, Check, Layers, DollarSign, ImagePlus, Sparkles as SparklesIcon, Trash2, RotateCcw } from 'lucide-react';
...
export function HistoryPanel({
    ...
    const openLightbox = (item: HistoryMetadata) => {
        ...
        const media: LightboxMedia[] = mediaItems
            .map((m) => ({
    ...
    return (
        <Card className='border-border bg-card flex h-full w-full flex-col overflow-hidden rounded-md border shadow-[0_1px_0_0_var(--border)]'>
            <CardHeader className='border-border flex flex-row items-start justify-between gap-4 border-b px-5 py-4'>
                <div className='flex items-center gap-3'>
                    <CardTitle className='font-display text-foreground text-3xl leading-none font-normal tracking-tight'>
                        History
                    </CardTitle>
                </div>
                {history.length > 0 && (
                    <Button
                        ...
                        className='text-muted-foreground hover:bg-muted/60 hover:text-foreground h-auto rounded-md px-2 py-1'>
                        Clear
                    </Button>
                )}
            </CardHeader>
            <CardContent className='flex-grow overflow-y-auto p-4'>
                                ...
                                <div key={itemKey} className='flex flex-col'>
                                    <div className='group relative'>
                                        <button
                                            ...
                                            className='border-border group-hover:border-foreground/40 focus:ring-ring relative block aspect-square w-full overflow-hidden rounded-t-md border transition-all duration-150 focus:ring-2 focus:ring-offset-2 focus:ring-offset-black focus:outline-none'
                                            ...
                                            {thumbnailUrl ? (
                                                ...
                                                <div className='bg-muted text-muted-foreground/80 flex h-full w-full items-center justify-center'>
                                                    ?
                                                </div>
                                            )}
                                            {item.mode === 'video' && (
                                                <div className='bg-foreground/70 text-background pointer-events-none absolute top-1 left-1 z-10 flex items-center gap-1 rounded-full px-1.5 py-0.5 font-mono text-[10px] tracking-[0.14em] uppercase backdrop-blur-sm'>
                                                    <SparklesIcon size={12} />
                                                    Video
                                                </div>
                                            )}
                                            {isMultiImage && (
                                                <div className='bg-background/70 text-foreground pointer-events-none absolute right-1 bottom-1 z-10 flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[12px]'>
                                                    <Layers size={16} />
                                                    {mediaCount}
                                                </div>
                                            )}
                                        ...
                                        {!isVideo && firstMedia && onSendToEdit && (
                                            <button
                                                ...
                                                className='border-primary/40 bg-primary/90 text-primary-foreground absolute inset-x-0 bottom-0 z-20 flex items-center justify-center gap-1.5 border-t px-2 py-1.5 font-mono text-[10px] tracking-[0.18em] uppercase opacity-0 backdrop-blur-sm transition-opacity duration-150 group-hover:opacity-100 focus:opacity-100 focus:outline-none'>
                                                <ImagePlus size={11} />
                                                Reference
                                            </button>
                                        )}
                                    ...
                                    <div className='border-border bg-background text-muted-foreground space-y-1 rounded-b-md border border-t-0 p-2 text-xs'>
                                        <p title={`Generated on: ${new Date(item.timestamp).toLocaleString()}`}>
                                            <span className='text-foreground/90 font-medium'>Time:</span>{' '}
                                            {formatDuration(item.durationMs)}
                                        </p>
                                        <p>
                                            <span className='text-foreground/90 font-medium'>Model:</span>{' '}
                                            {item.model || (isVideo ? 'sora-2' : 'Unknown')}
                                        </p>
                                        {isVideo ? (
                                            <>
                                                <p>
                                                    <span className='text-foreground/90 font-medium'>Resolution:</span>{' '}
                                                    {item.videoSize}
                                                </p>
                                                <p>
                                            ...
                                            <>
                                                <p>
                                                    <span className='text-foreground/90 font-medium'>Quality:</span>{' '}
                                                    {item.quality}
                                                </p>
                                            </>
                                        )}
                                                ...
                                                <div className='mt-1 flex flex-wrap gap-1'>
                                                    {referenceFilenames.map((refFilename) => {
                                                        ...
                                                        return src ? (
                                                            ...
                                                            <div
                                                                key={refFilename}
                                                                className='border-border bg-muted text-muted-foreground flex h-8 w-8 items-center justify-center rounded border text-[9px]'>
                                                                ?
                                                            </div>
                                                        );
                                                    })}
                                        ...
                                        <div className='mt-2 flex items-center gap-1'>
                                            <Dialog
                                                ...
                                                <DialogTrigger asChild>
                                                    <button
                                                        type='button'
                                                        className='border-border bg-muted/30 text-muted-foreground hover:bg-muted/60 hover:text-foreground flex-grow cursor-pointer rounded-md border px-2 py-1.5 text-left text-[11px] leading-snug transition-colors'
                                                ...
                                                <DialogContent className='border-border bg-popover text-foreground sm:max-w-[625px]'>
                                                    ...
                                                    <div className='border-border bg-muted text-foreground/90 max-h-[400px] overflow-y-auto rounded-md border p-3 py-4 text-sm'>
                                                        {item.prompt || 'No prompt recorded.'}
                                                    </div>
                                                    <DialogFooter>
                                                        <Button
                                            ...
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <Button
                                                        ...
                                                        className='border-border bg-background text-muted-foreground hover:bg-muted/60 hover:text-foreground h-6 w-6 disabled:opacity-40'>
                                                        <RotateCcw size={13} />
                                                    </Button>
                                                </TooltipTrigger>
                                            ...
                                            <Dialog
                                                ...
                                                <DialogTrigger asChild>
                                                    <Button
                                                        className='bg-destructive/70 text-foreground hover:bg-destructive/70 h-6 w-6'
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                ...
                                                <DialogContent className='border-border bg-popover text-foreground sm:max-w-md'>
                                                    ...
                                                    <div className='flex items-center space-x-2 py-2'>
                                                        ...
                                                        <label
                                                            ...
                                                            className='text-foreground/90 text-sm leading-none font-medium peer-disabled:cursor-not-allowed peer-disabled:opacity-70'>
                                                            Don&apos;t ask me again
                                                        </label>
                                                    </div>
                ...
                {/* Lightbox */}
                <ImageLightbox media={lightboxMedia} open={lightboxOpen} onOpenChange={setLightboxOpen} />
            </CardContent>
        </Card>
    );
}
